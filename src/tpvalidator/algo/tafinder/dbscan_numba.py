...
@njit(int32[::1](nb.float32[::1], nb.float32[::1], nb.int64, nb.int64))
...
def apply_dbscan(window_tps, epsilon=2, min_samples=2):
    ...
    cluster_num_tps = np.array([len(c) for c in clusters])

    max_cluster_idx = np.argmax(cluster_sums)

    result = pd.Series({
        "n_clusters": len(cluster_ids),
        "mean_cluster_sadc": cluster_sums.mean(),
        "total_cluster_sadc": cluster_sums.sum(),
        "max_cluster_sadc": cluster_sums[max_cluster_idx],
        "max_cluster_num_tps": cluster_num_tps[max_cluster_idx],
        "tp_index": df_index,
