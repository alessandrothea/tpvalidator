...
def format_val_err(val, err, sig_figs=2):
    """
    Format value and error as (val ± err) × 10^X string.
    The exponent X is chosen from the magnitude of the value.
    """
    if not np.isfinite(val) or not np.isfinite(err):
        return f"{val:.3g} ± {err:.3g}"
    
    if val == 0:
        return f"(0 ± {err:.{sig_figs}g})"
    ...
    if exp == 0:        return f"({val_scaled:.{sig_figs}f} ± {err_scaled:.{sig_figs}f})"
    else:
        return f"({val_scaled:.{sig_figs}f} ± {err_scaled:.{sig_figs}f})×$10^{exp}$"