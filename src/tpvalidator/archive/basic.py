...
from ..tpc_angles import calculate_angles, calculate_more_angles
from ..utils import compute_histogram_ratio
from ..rootio import read_data as load_data, read_event_list as load_event_list, TriggerNtupleReader
...
def load_info(data_path: Path) -> dict:
...
def equalize_ranges(df: pd.DataFrame) -> pd.DataFrame:
    """Return equal-span [min, max] per column so 3D plot axes share the same scale."""
    centers = (df.min() + df.max()) / 2
    half_span = (df.max() - df.min()).max() / 2
    return pd.DataFrame(
        {col: [centers[col] - half_span, centers[col] + half_span] for col in df.columns}
    )
...
class BasicTPData:
    ...
    def tp_threshold(self, plane: int):
        if plane in [0,1,2]:
            ...
            raise ValueError(f"Invalid plane id: {plane}")
    ...
    def tp_backtracker_offset(self, plane: int):
        ...
        if plane not in plane_map:
            raise KeyError(f"Plane '{plane}' not known")
...
def plot_3dev_points_of_origin(tp_data: BasicTPData, figsize=a4_landscape, title_size=None) -> Figure:
    ...
    $E_{{kin}}={float(ev_row['Ekin'].values[0]):.2f}$ MeV
    $P_{l}$={float(Pax.values[0])*1000:.2f} MeV
    Particle origin: ({float(ev_row['startX'].values[0]):.1f}, {float(ev_row['startY'].values[0]):.1f}, {float(ev_row['startZ'].values[0]):.1f})
    """)
...
def plot_charge(tp_data: BasicTPData, title_size=None, figsize=a4_landscape, **kwargs) -> Figure:
    #-----
    ...
    x.hist(bins=100, figsize=figsize, layout=(3,3), color='xkcd:dark salmon', **kwargs)
    fig = plt.gcf()
...
def main(data_path, interactive, output) -> None:
    ...
    report_name = output if output is not None else f'tp_val_{data_path.stem}.pdf'
