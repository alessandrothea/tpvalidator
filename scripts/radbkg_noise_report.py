...
def prepare_figures(ws: workspace.TriggerPrimitivesWorkspace, output_dir: Path) -> dict:
    ...
    pf = Portfolio(output_dir, 'radbkg')
...
def write_report(figures_dir, report_file):
    ...
    pdf.image( figures_dir / f'{sig_lbl}_adc_dist.svg', w=pdf.epw)
    ...
    pdf.image( figures_dir / f'{sig_lbl}_xyz_pos_dist_all_tps.png', h=pdf.eph*0.9, x=Align.L)
    ...
    ii = pdf.image( figures_dir / f'{sig_lbl}_event_10_peak26_all_tps.png', w=img_w, x=Align.L,)
    ...
    ii = pdf.image( figures_dir / f'{sig_lbl}_event_10_peak36_all_tps.png', w=img_w)
    # pdf.mark_cursor(text="I3")

    pdf.set_xy(x0, y0+ii.rendered_height)
    x0, y0 = pdf.get_x(), pdf.get_y()

    ii = pdf.image( figures_dir / f'{sig_lbl}_event_10_peak46_all_tps.png', w=img_w)
    # pdf.mark_cursor(text="I4")
    pdf.set_xy(x0+img_w, y0)
    ii = pdf.image( figures_dir / f'{sig_lbl}_event_10_peak56_all_tps.png', w=img_w)
    pdf.set_xy(x0, y0+ii.rendered_height)
    ...
    pdf.image( figures_dir / f'{sig_lbl}_x_pos_dist_all_tps.svg', w=pdf.epw, x=Align.C)
    pdf.write_markdown(
    ...
    pdf.image( figures_dir / f'{sig_lbl}_x_pos_weighted_dist_all_tps.svg', w=pdf.epw, x=Align.C)
    ...
    ii = pdf.image( figures_dir / f'{sig_lbl}_start_time_dist_all_tps.svg', w=pdf.epw*0.9, x=Align.L)
    ii = pdf.image( figures_dir / f'{sig_lbl}_ides_time_dist_all_tps.svg', h=pdf.eph*0.3, x=Align.L)
    ...
    pdf.image( figures_dir / f'{sig_lbl}_start_time_dist.svg', w=pdf.epw*0.9, x=Align.L)
    pdf.write_markdown("""
    ...
    pdf.image( figures_dir / f'{sig_lbl}_vs_elnoise_var_dist.svg', w=pdf.epw, x=Align.C)
    # pdf.write_html("""
    #                <ul>
    ...
    pdf.image( figures_dir / f'{sig_lbl}_peakadc_dist_in_drift_bins.svg', w=pdf.eph, x=Align.C)


    # ---------------------------------------------------------------------
    pdf.add_page()
    pdf.write_markdown("# **Distribution of TP TOT across the drift**\nBins of trueX - plane 2 (collection)", tag_styles=tag_styles)
    pdf.image( figures_dir / f'{sig_lbl}_tot_dist_in_drift_bins.svg', w=pdf.eph, x=Align.C)

    # ---------------------------------------------------------------------
    pdf.add_page()
    pdf.write_markdown("# **Distribution of TP SADC across the drift**\nBins of trueX - plane 2 (collection)", tag_styles=tag_styles)
    pdf.image( figures_dir / f'{sig_lbl}_sadc_dist_in_drift_bins.svg', w=pdf.eph, x=Align.C)
    ...
    x = pdf.get_x()
    y = pdf.get_y()
    pdf.image( figures_dir / f'{sig_lbl}_peak_dist_stack_in_drift_bins.svg', w=pdf.epw//3)
    pdf.set_y(y)
    pdf.image( figures_dir / f'{sig_lbl}_tot_dist_stack_in_drift_bins.svg', w=pdf.epw//3, x=x+pdf.epw//3)
    pdf.set_y(y)
    pdf.image( figures_dir / f'{sig_lbl}_sadc_dist_stack_in_drift_bins.svg', w=pdf.epw//3, x=x+2*pdf.epw//3)
    pdf.write_markdown("""
    ...
    pdf.image( figures_dir / f'{sig_lbl}_dists_with_peakadc_cuts.svg', h=pdf.eph*0.9, x=Align.L)
    ...
    pdf.image( figures_dir / f'{sig_lbl}_dists_with_tot_cuts.svg', h=pdf.eph*0.9, x=Align.L)
    ...
    pdf.image( figures_dir / f'{sig_lbl}_dists_with_sadcs_cuts.svg', h=pdf.eph*0.9, x=Align.L)
    ...
    pdf.image( figures_dir / f'{sig_lbl}_peak_thresh_scan_perf.svg', w=pdf.epw*0.95, x=Align.C)
    ...
    pdf.image( figures_dir / f'{sig_lbl}_tot_thresh_scan_perf.svg', w=pdf.epw*0.95, x=Align.C)
    ...
    pdf.image( figures_dir / f'{sig_lbl}_sadc_thresh_scan_perf.svg', w=pdf.epw*0.95, x=Align.C)
...
def main(tp_file_path : str, wf_file_path: str, event_range=None, make_figures:bool=True, interactive: bool=False):
    ...
    report_dir = Path('./reports/radbkg/')
...
if __name__ == '__main__':
    tp_tree_file = 'data/vd/bkg/100events/tptree_st_tpg_vd_radiols.root'
    wf_file = 'data/vd/bkg/100events/trigger_digits_waves_detsim_vd_radiols.root'
